// Sidenav
export const GET_ACTIVE_NAV = 'GET_ACTIVE_NAV';
export const SET_ACTIVE_NAV = 'SET_ACTIVE_NAV';

// Map
export const SET_MAP_VIEW = 'SET_MAP_VIEW';
export const SET_MAP_TOOLS = 'SET_MAP_TOOLS';
export const SET_ACTIVE_TOOL = 'SET_ACTIVE_TOOL';
export const SET_ACTIVE_TOOL_MENU = 'SET_ACTIVE_TOOL_MENU';
export const SET_ACTIVE_FEATURE_MODE = 'SET_ACTIVE_FEATURE_MODE';
export const SET_SNAPPING_FEATURE_SOURCES = 'SET_SNAPPING_FEATURE_SOURCES';

export const REMOVE_LAYER_FROM_VIEW = 'REMOVE_LAYER_FROM_VIEW';

// Layers
export const GET_LAYER_GROUPS = 'GET_LAYER_GROUPS';
export const GET_LAYER_GROUPS_FULFILLED = 'GET_LAYER_GROUPS_FULFILLED';

export const SET_LAYER_LIST = 'SET_LAYER_LIST';
export const TOGGLE_LAYER = 'TOGGLE_LAYER';
export const UPDATE_LAYER = 'UPDATE_LAYER';
export const UPDATE_LAYER_FIELDS = 'UPDATE_LAYER_FIELDS';
export const DEACTIVATE_LAYER = 'DEACTIVATE_LAYER';

export const GET_ACTIVE_LAYER_TAB = 'GET_ACTIVE_LAYER_TAB';
export const SET_ACTIVE_LAYER_TAB = 'SET_ACTIVE_LAYER_TAB';

export const HIDE_LAYER = 'HIDE_LAYER';
export const ADD_SEARCH_RESULTS_LAYER = 'ADD_SEARCH_RESULTS_LAYER';

export const SET_GRAPH_LAYER = 'SET_GRAPH_LAYER';

export const ADD_USER_LAYER = 'ADD_USER_LAYER';

export const TOGGLE_LAYER_LEGEND = 'TOGGLE_LAYER_LEGEND';

export const TOGGLE_LAYER_VISIBLE_ZOOM_OUT = 'TOGGLE_LAYER_VISIBLE_ZOOM_OUT';

// Map overview
export const TOGGLE_INDEX_MAP = 'TOGGLE_INDEX_MAP';

// Map config
export const GET_MAP_CONFIG = 'GET_MAP_CONFIG';
export const GET_MAP_CONFIG_FULFILLED = 'GET_MAP_CONFIG_FULFILLED';
export const SET_SCALE = 'SET_SCALE';

// Table
export const TOGGLE_TABLE = 'TOGGLE_TABLE';
export const TOGGLE_FILTER = 'TOGGLE_FILTER';

export const SELECT_FEATURES = 'SELECT_FEATURES';
export const DE_SELECT_SELECTED_FEATURES = 'DE_SELECT_SELECTED_FEATURES';
export const TOGGLE_SELECTION = 'TOGGLE_SELECTION';
export const TOGGLE_SELECT_ALL = 'TOGGLE_SELECT_ALL';
export const SET_ROW_FILTER = 'SET_ROW_FILTER';
export const REMOVE_TABLE_FEATURE = 'REMOVE_TABLE_FEATURE';

export const SET_ACTIVE_TABLE = 'SET_ACTIVE_TABLE';
export const SET_COLUMNS = 'SET_COLUMNS';

export const CLEAR_TABLE_DATA = 'CLEAR_TABLE_DATA';
export const CLOSE_LAYER = 'CLOSE_LAYER';
export const CLEAR_SEARCH_DATA = 'CLEAR_SEARCH_DATA';
export const SET_EDITED_LAYER = 'SET_EDITED_LAYER';

export const SET_SINGLE_LAYER_GEOMETRY = 'SET_SINGLE_LAYER_GEOMETRY';

export const APPLY_EDITS = 'APPLY_EDITS';
export const APPLY_DELETED_FEATURES = 'APPLY_DELETED_FEATURES';
export const TABLE_EDITED = 'TABLE_EDITED';

export const SET_BUTTON_AMOUNT = 'SET_BUTTON_AMOUNT';

export const SET_ACTIVE_PAGE = 'SET_ACTIVE_PAGE';

export const ADD_FILTERED = 'ADD_FILTERED';

export const ADD_FEATURE_NO_GEOMETRY = 'ADD_FEATURE_NO_GEOMETRY';

// Search
export const SEARCH_FEATURES = 'SEARCH_FEATURES';
export const SEARCH_FEATURES_FULFILLED = 'SEARCH_FEATURES_FULFILLED';

export const SET_SEARCH_STATE = 'SET_SEARCH_STATE';
export const SET_SEARCH_OPTIONS = 'SET_SEARCH_OPTIONS';

export const SET_ACTIVE_SEARCH = 'SET_ACTIVE_SEARCH';

// Modal
export const SET_ACTIVE_MODAL = 'SET_ACTIVE_MODAL';

// Admin tools
export const SET_ACTIVE_ADMIN_TOOL = 'SET_ACTIVE_ADMIN_TOOL';
export const DELETE_SELECTED_DATA = 'DELETE_SELECTED_DATA';

// User layer
export const REMOVE_USER_LAYER = 'REMOVE_USER_LAYER';
export const REMOVE_USER_LAYER_FULFILLED = 'REMOVE_USER_LAYER_FULFILLED';

// Workspace
export const GET_WORKSPACE_LIST = 'GET_WORKSPACE_LIST';
export const GET_WORKSPACE_LIST_FULFILLED = 'GET_WORKSPACE_LIST_FULFILLED';
export const GET_WORKSPACE_LIST_REJECTED = 'GET_WORKSPACE_LIST_REJECTED';
export const SET_WORKSPACE = 'SET_WORKSPACE';
export const SET_WORKSPACE_FULFILLED = 'SET_WORKSPACE_FULFILLED';
export const SET_WORKSPACE_REJECTED = 'SET_WORKSPACE_REJECTED';

// Shapefile
export const ADD_SHAPEFILE_LAYER = 'ADD_SHAPEFILE_LAYER';

// Confirm modal
export const SHOW_CONFIRM_MODAL = 'SHOW_CONFIRM_MODAL';
export const HIDE_CONFIRM_MODAL = 'HIDE_CONFIRM_MODAL';

// Map draw
export const SET_MAP_DRAW_TEXT = 'SET_MAP_DRAW_TEXT';
export const SET_HAS_GRAPHICS = 'SET_HAS_GRAPHICS';
export const TOGGLE_MEASUREMENTS = 'TOGGLE_MEASUREMENTS';

// Contract
export const SET_CONTRACT_LIST_INFO = 'SET_CONTRACT_LIST_INFO';
export const REMOVE_CONTRACT_LIST_INFO = 'REMOVE_CONTRACT_LIST_INFO';

// Property
export const SET_PROPERTY_INFO = 'SET_PROPERTY_INFO';
export const SET_PROPERTY_INFO_FULFILLED = 'SET_PROPERTY_INFO_FULFILLED';
export const SET_PROPERTY_INFO_REJECTED = 'SET_PROPERTY_INFO_REJECTED';

export const SET_PROPERTY_INFO_LINKS = 'SET_PROPERTY_INFO_LINKS';
export const SET_PROPERTY_INFO_LINKS_FULFILLED = 'SET_PROPERTY_INFO_LINKS_FULFILLED';
export const SET_PROPERTY_INFO_LINKS_REJECTED = 'SET_PROPERTY_INFO_LINKS_REJECTED';

export const CLEAR_PROPERTY_INFO = 'CLEAR_PROPERTY_INFO';
export const TOGGLE_PROPERTY_AREA_SEARCH = 'TOGGLE_PROPERTY_AREA_SEARCH';

// User
export const SET_USER_INFO = 'SET_USER_INFO';

// Offline
export const ADD_EDIT = 'ADD_EDIT';
export const SET_EDITS = 'SET_EDITS';

// Loading
export const SET_LOADING = 'SET_LOADING';
export const REMOVE_LOADING = 'REMOVE_LOADING';

export const SET_LOADING_LAYERS = 'SET_LOADING_LAYERS';
export const REMOVE_LOADING_LAYER = 'REMOVE_LOADING_LAYER';
export const REMOVE_LOADING_LAYERS = 'REMOVE_LOADING_LAYERS';

// Portal
export const TOGGLE_PORTAL = 'TOGGLE_PORTAL';
